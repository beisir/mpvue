<template lang="html">
    <div>
        <div class="tra-container">
            ...
            <div class="tra-time">
                <div class="tra-input">
                    <span class="tra-btn" @click="scrollChange">查询</span>
                </div>
            </div>
        </div>
        <scroll-view
            ...
            <ul class="tra-detail">
                <li v-for="(abroadItem, abroadIndex) in abroadList" :key="abroadItem.trackType">
                    <p>车皮/集装箱号：{{abroadItem.containerNo}}（{{abroadItem.queryState}}）</p>
                    <div class="tra-pls">
                        <span>{{abroadItem.trackStatus}}</span>
                        <a :href="'/pages/feedback/main?traQueryId=' + abroadItem.internaId + '&active=' + currentTab">查看追踪详情</a>
                    </div>
